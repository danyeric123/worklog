...
@cli.command()
@click.option('--git/--no-git', default=None, help='Automatically commit changes to git')
@click.option('--date', type=click.DateTime(formats=["%Y-%m-%d"]), 
              default=str(date.today()), help='Override date (YYYY-MM-DD)')
def log(git, date):
...
def view(month):
    ...
    try:
        file_path = get_log_file_path() if not month else \
                   os.path.join(create_log_directory(), f"{month.strftime('%Y_%m')}.md")
